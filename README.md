...
- 运单：登记与维护运单（客户、寄件城市、收件城市、实际重量、体积、件数、保价金额、附加服务、状态、创建时刻），支持按关键词、客户、账期、状态筛选，可以只看收件城市还没归属分区的运单；选中账期会显示该账期按北京时间的起止时刻
- 运单计费：对单条运单算一次费用，结果会记在这条运单上，页面上直接能看到上次算出来的数
- 分区：维护分区编码、名称、覆盖城市与城市别名、首重与续重价格、偏远附加、启用状态
- 客户：维护客户编码、名称、结算方式（月结／现结）、折扣、账期日
- 账单：出账前可先「核对账期运单」预览条数与最早、最晚创建时刻；按账期与客户出账，查看账期起止、账单总额与逐条明细，可以把账单作废
...
6. 账期按运单创建时刻的**北京时间（UTC+8）年月**归属：当月 1 日 00:00（含）至次月 1 日 00:00（不含）为一个账期；不按协调世界时（UTC）取月份，月初凌晨与月末深夜的边界运单按本地日期落在正确账期。页面录入的创建时刻不带时区时按北京时间理解。出账时账单会记下该账期运单的条数与实际最早、最晚创建时刻供核对；运单进账单之后会被锁定，不能再直接删改
...
POST   /api/bills/preview        POST /api/bills/generate
...
- `GET /api/periods` 返回每个账期的北京时间起止（`startAtText`/`endAtText`）、运单条数与最早、最晚创建时刻
- `GET /api/waybills?period=2026-09` 按账期筛运单；运单对象带 `period` 字段（创建时刻的北京年月）
- `POST /api/bills/preview` 不出账，先返回该账期该客户将纳入的运单条数、最早/最晚创建时刻与预计金额，供出账前核对
